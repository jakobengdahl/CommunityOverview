...
| `icon` | ✅ toolbox create (shared picker slot), move, rotate (right-click) and attach by dragging near a node/annotation; right-click picker grid over the full icon vocabulary changes an existing icon's name — renders every one of the 75 host-registry icon names as its own distinct glyph (see [Canvas rendering](#canvas-rendering)) without a background/border, plus layer | ✅ generic tool set | ✅ | ✅ | ✅ | ⬜ |
| `vote_dot` | ✅ toolbox create, move, rotate/layer (right-click) | ✅ generic tool set | ✅ | ✅ | ✅ | ⬜ |
