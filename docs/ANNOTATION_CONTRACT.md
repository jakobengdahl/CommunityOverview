...
| `shape` | ✅ toolbox creates all six variants, each drawn distinctly; right-click editor changes an existing shape's subtype, colour, rotation and layer (front/back). `triangle`, `rhombus` and `hexagon` are created at the ratio that makes their percentage clip-path draw equal-length sides, and their resize keeps that ratio; `rectangle`, `circle` and `process_arrow` fill whatever box they are given, so a `circle` in a non-square box is an ellipse | ✅ generic tool set (`content.shape`) | ✅ | ✅ | ✅ | ⬜ |
