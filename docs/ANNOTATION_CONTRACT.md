...
offers it no handles, but the MCP tools do accept a `w`/`h` patch — that
patch is stored and reported back while changing nothing a viewer sees,
exactly like the `rotation` below. Scaling the sampled points instead is a
change to what a stored `w`/`h` means, so it would have to amend this
paragraph rather than quietly reinterpret existing strokes. A locked annotation of any
