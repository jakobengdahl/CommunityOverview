...
it. `freehand`, `label` and `line` are the three types whose stored
`geometry.w`/`h` a real box never reads for layout — a label sizes itself
from its text, a line from its endpoints, a stroke from its sampled points —
which used to make it easy to drop on the canvas leg without any visible
symptom: `freehandAnnotationToOverlay`, and the `label` and `line` branches
beside it in `sessionAnnotations.js`, carried no size across at all, unlike
`genericAnnotationToOverlay`, which does for all five generic kinds. Hydrating
one of the three reset its stored box to the model's 160×96 default (or
discarded an agent-set non-default one the same way) on the very next
autosave that shipped the annotation — a browser already connected when the
patch landed folded the server's real geometry into its baseline while its
overlay dropped it, so the reset shipped on the next autosave for any reason
at all, not only a touch to that annotation; a browser that loaded afterwards
needed an actual edit to it first. Saving a view lost it unconditionally too —
the document was rebuilt from the canvas overlays, which never carried the
box (`handleConfirmSaveView` → `legacyMetadataToAnnotationDocument`). `rotation`
was not affected for any of the three: the same translators did carry that
across in both directions. This was the "unsized-geometry clobber" class of
bug already closed for `icon`/`vote_dot`/`text`
(`smallfix-annotation-unsized-generic-geometry-clobber`) and is now closed for
these three too (`smallfix-browser-clobbers-unsized-annotation-geometry`):
`geometry.w`/`h` — including an explicit `0`, which is a real stored value
here, not a missing one — now survives both `sessionAnnotations.js`'s
server-document translators and the shared `overlayToFlowNode`/
`flowNodeToOverlay` leg in `packages/ui-graph-canvas` (the same `data.size`
data-only slot the generic-kind fix introduced, extended to `label`/`line`;
`freehand` was already a `GENERIC_OVERLAY_TYPES` member in that file and so
already had the slot), and the saved-view path is fixed for free since it is
built from the same translators. None of this makes any of the three visually
resizable — the box still drives nothing on screen for any of them — so
scaling the sampled points on a `w`/`h` patch remains open exactly as before;
what changed is only that the value an agent sets now survives contact with a
browser instead of being silently overwritten. A locked annotation of any
generic kind hides its resize handles the same way a locked `note` does.

`freehand` has a second, narrower and still-open gap in the same area, out of
scope for `smallfix-browser-clobbers-unsized-annotation-geometry`:
`freehandAnnotationToOverlay` anchors the overlay's `position` to `points[0]`
(so a plain ReactFlow drag can move the whole stroke by only updating
`position`, without rewriting every sampled point), and
`freehandOverlayToAnnotation` writes `position`/`geometry.x`/`geometry.y`
back from that same anchor on the way in. An agent-created stroke whose
envelope position differs from its first sample (e.g. `position: {x: 100, y:
50}` with `points` sampled elsewhere) has that envelope position silently
replaced by `points[0]` on the very first round trip. This is arguably a
normalisation — the envelope position is redundant with `points[0]` once the
points are absolute, and nothing currently reads the envelope position
independently for a stroke — rather than a data-loss defect the way the
`w`/`h` clobber was, and fixing it would touch the drag-by-position
convention used throughout the canvas's freehand handling, a materially
different and larger change than carrying an opaque `size` value through.
Left open here deliberately rather than folded into this fix; tracked as its
own follow-up.
...
| `label` | ✅ toolbox create, inline edit, drag, rotate/recolor/resize-text/layer/duplicate (right-click; no colour chosen defaults to `#64748b`, `LabelNode.jsx`'s `DEFAULT_LABEL_COLOR`), attach by dragging near a node/annotation or, at creation time, via the "nearby object menu" (right-click an eligible node/annotation's own menu, "Add nearby" → Label) — previously listed "attach" as done, but it was modeled server-side only and never wired into the canvas translation layer until this slice | ✅ generic tool set | ✅ no translator drop remains. `geometry.w`/`h` used to be reset to the model's 160×96 default by the next autosave that shipped the label and by any saved view — fixed (`smallfix-browser-clobbers-unsized-annotation-geometry`): the value (including an explicit `0`) now survives both `sessionAnnotations.js` and the shared canvas translator; only an agent can set it, since a `label` has no resize handles (this row previously claimed "resize" here — corrected, `smallfix-contract-label-row-claims-resize-it-lacks`). The overlay also used to carry only `color` and `fontSize` out of `style`, dropping any other style key an agent set; task-annotation-responsive-bottom-toolbox's opacity control closed it for `opacity`, and every remaining key now rides along as the overlay's `extraStyle` (through both `sessionAnnotations.js` and the shared canvas translator) and is merged back under the named keys (`smallfix-label-overlay-drops-nonvisual-style-keys`, fixed). `text`, colour, font size, opacity, every other `style` key, `attachment`, `rotation`, `z`, `locked` and `geometry.w`/`h` survive | ✅ | ✅ | ⚠ audited 2026-08-30, updated same day (see [audit](#keyboard-touch-and-screen-reader-controls-audit-v1-accessibility-baseline)): same as `note` — a visible Edit button now opens the menu. **Update 2026-08-30 (task-annotation-accessible-shared-controls):** now has a designed accessible name ("Label, {text}"), Shift+F10 reachability, menu arrow-nav/focus-trap, an "Attach to…"/Detach pair and selected-and-focused-on-create; screen-reader/physical-device verification still deferred |
| `line` | ⚠ toolbox create, endpoint attach/drag, recolor/layer/duplicate/unlock (right-click; no colour chosen defaults to `#111827`, `ArrowNode.jsx`'s `DEFAULT_ARROW_COLOR`); a `rotation` the MCP tools accept is stored and reported but never drawn | ✅ generic tool set (`arrow` alias) | ✅ no translator drop remains. `geometry.w`/`h` used to be rewritten to the model's 160×96 default by the next autosave that shipped the line and by any saved view — fixed (`smallfix-browser-clobbers-unsized-annotation-geometry`): the value (including an explicit `0`, which an agent-created line is stored at by default — `build_annotation` defaults `w`/`h` to `0` — since `ArrowNode` sizes itself from the endpoints, not this field) now survives both `sessionAnnotations.js` and the shared canvas translator. The overlay also used to drop the model's `start`/`end` endpoint descriptors, rebuilding an `attachment` an agent set on either endpoint (see [Attachment and detach behavior](#attachment-and-detach-behavior)) as a bare point — fixed (`smallfix-line-endpoint-attachment-dropped-by-translator`): an attached endpoint is now carried through both translators. And it used to carry only `color` out of `style` — the identical branch the `label` row above carries, and fixed by the same item (`smallfix-label-overlay-drops-nonvisual-style-keys`, whose id reads label-only): `opacity` as a named field, every other key as `extraStyle`. `rotation`, `z`, `locked`, `geometry.w`/`h`, arrowheads, colour, opacity, every other `style` key, endpoint attachments and the GUI's own anchors all survive | ✅ | ✅ | ⚠ audited 2026-08-30, updated same day (see [audit](#keyboard-touch-and-screen-reader-controls-audit-v1-accessibility-baseline)): a visible Edit button now opens the menu. **Update 2026-08-30 (task-annotation-accessible-shared-controls):** now has a designed accessible name ("Arrow", a fixed word — a line has no caption to include), Shift+F10 reachability and menu arrow-nav/focus-trap; not one of `ATTACHABLE_OVERLAY_KINDS` (it attaches per-endpoint, its own separate mechanism, unchanged) so it gets no "Attach to…"/Detach pair; screen-reader/physical-device verification still deferred |
