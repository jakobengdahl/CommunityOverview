...
| `GRAPH_POSTGRES_SCOPE` | *(unset)* | An opaque identifier tagging this instance's rows; see *Keeping scopes apart* |
...
CREATE TABLE <schema>.graph_nodes (
  ...
  -- every row of every store written before this column existed. See
  -- "Keeping scopes apart" below; a store that sets no GRAPH_POSTGRES_SCOPE
  -- leaves it NULL and never reads it.
  scope_id text
);
...
- **Every row this instance writes carries the value.** Both write paths — the
  whole-graph save and the entity upsert — stamp it, including a row that
  carried none before, so a store does not drift into two kinds of row.
...
- **`graph_metadata` is not covered.** Its primary key is a column that can
  only hold true, so it holds one row for the whole store and has nowhere to
  put a second scope's. A column there would suggest an isolation the shape
  cannot deliver.
- **`id` remains the primary key of each entity table**, so two scopes sharing
  one table cannot both hold `n0`, and an upsert of an id conflicts with *the*
  row of that id whatever scope it carries. Change notification is per schema
  too (the channel is derived from it), so two scopes behind one set of tables
  would hear each other's entity ids announced. The seam is the row-level layer
  *underneath* the separation this backend already offers — a schema per
