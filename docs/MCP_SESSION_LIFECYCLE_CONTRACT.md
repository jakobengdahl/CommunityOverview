...
Tool results are structured (never a bare exception). The `error` values below
are the literal strings the open-core tools return (`backend/service/mcp_tools.py`);
where a row also names a `message`, that field carries the human-readable text.
...
| Not authorized (SaaS) | `{"success": false, "error": "Graph access denied", "error_code": "access_denied", "message": <hook reason>, "authorization": {…}}` |
...
Rename and delete each draw one unit per call from a rate budget of their own.
