...
describe('overlay serialization', () => {
  it('round-trips a note (size <-> style, font size)', () => {
    const overlay = { id: 'note-1', kind: 'note', position: { x: 1, y: 2 }, text: 'hi', color: '#FEF08A', fontSize: 18, size: { w: 200, h: 140 } };
    const node = overlayToFlowNode(overlay);
    expect(node).toMatchObject({
      id: 'note-1', type: 'note', position: { x: 1, y: 2 },
      style: { width: 200, height: 140 }, data: { text: 'hi', color: '#FEF08A', fontSize: 18 },
    });
  ...
  it('round-trips a label with a font size', () => {
    const overlay = { id: 'label-1', kind: 'label', position: { x: 3, y: 4 }, text: 'L', color: '#fff', fontSize: 28 };
  ...
  it('round-trips a double-headed line with anchors', () => {
    const overlay = {
      id: 'arrow-2', kind: 'arrow', position: { x: 0, y: 0 }, dx: 100, dy: 0, color: '#fff',
  ...
  it('leaves a free (unanchored) arrow draggable', () => {
    const node = overlayToFlowNode({ id: 'arrow-3', kind: 'arrow', position: { x: 0, y: 0 }, dx: 10, dy: 10 });
...
describe('nodeCenter', () => {
  it('uses measured size when available', () => {
    expect(nodeCenter({ position: { x: 0, y: 0 }, width: 100, height: 40 })).toEqual({ x: 50, y: 20 });
  });

  it('prefers positionAbsolute over position', () => {
    expect(nodeCenter({ position: { x: 0, y: 0 }, positionAbsolute: { x: 10, y: 10 }, width: 20, height: 20 }))
      .toEqual({ x: 20, y: 20 });
  });
...
describe('resolveAnchoredArrow', () => {
  ...
  it('glues the start endpoint to its target centre and keeps the far end put', () => {
    ...
    expect(resolveAnchoredArrow(arrow, centers)).toEqual({ position: { x: 20, y: 5 }, dx: 80, dy: -5 });
  });
  ...
  it('glues the end endpoint to its target centre', () => {
    ...
    expect(resolveAnchoredArrow(arrow, centers)).toEqual({ position: { x: 0, y: 0 }, dx: 60, dy: 40 });
  });
