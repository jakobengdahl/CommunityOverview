<!-- PR target is always `dev` (hotfixes excepted — see CLAUDE.md). -->

## Summary

<!-- What changed and why. Reference the SMALL_FIXES.md entry or design doc
     section this implements, if any. -->

## What was not changed (scope)

<!-- Explicit boundaries: related issues deliberately left alone (and logged
     in SMALL_FIXES.md), follow-up steps that belong to a later PR. -->

## Test plan

<!-- Which tests cover this (paths), and how to verify manually.
     Full suite before opening the PR: pytest backend/ -q -->

## Screenshots affected

<!-- List `docs/images/<filename>.png` entries this change invalidates or
     requires, with a short why — or "None". Jakob captures screenshots. -->
