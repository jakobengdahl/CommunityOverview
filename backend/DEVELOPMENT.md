...
`rotation` is in degrees, and every type stores and reports back whatever is
written. The canvas *draws* it for text, label, note, image, icon, vote dot,
shape and frame only; a `line`'s or a `freehand` stroke's rotation is kept and
