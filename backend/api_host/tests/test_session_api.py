...
class TestSessionCrud:
    ...
    def test_rename_materialises_a_session_that_was_never_created(self, test_app: TestClient):
        ...
        sid = "9999-9997"
        test_app.delete(f"/api/sessions/{sid}")  # clean slate (shared temp dir across test runs)
        resp = test_app.patch(f"/api/sessions/{sid}", json={"name": "renamed"})
        assert resp.status_code == 200
        assert resp.json()["name"] == "renamed"
