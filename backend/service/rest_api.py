...
def _register_import_endpoints(
    ...
    async def import_graph_archive(
        ...
        - manifest present, every checksum it declares verified, and
          ``embedding_model`` matching this instance's live model: vectors are
          restored directly from ``embeddings.bin`` — bit-identical, no async
          job (``embeddings_status: "restored"``).
